...
class DemoApiPropertiesCreatePropertyKeepPropertyStructure {
    readonly DATAPOINT_NAME = '0:SampleObject.SampleServer';
    static init() {
        window.sigApiReadyPromise.then(() => {
            const codemodule = new DemoApiPropertiesCreatePropertyKeepPropertyStructure();
        });
    }
    ...
    constructor() {
        ...
        if (!datapoint) {
            console.log(`[cm_demo_api_properties_createProperty_keepPropertyStructure] sigApi.datapointManager.getDatapoint('${this.DATAPOINT_NAME}') failed!`);
            return;
        }
        const blueprint = window.sigApi.datapointManager.getDatapoint(this.DATAPOINT_NAME)?.getProperties().unit;
        if (!blueprint) {
            console.log(`[cm_demo_api_properties_createProperty_keepPropertyStructure] sigApi.datapointManager.getDatapoint('${this.DATAPOINT_NAME}').getProperties().unit failed!`);
            return;
        }
        ...
        this._property = window.sigApi.properties.createProperty(blueprint, predefPropMask, keepPropertyStructure);
        ...
        } else if (window.sigApi.properties.instanceofPropertyPropertySwitch(this._property)) {
            // scenario 3
        ...
        this._property?.addObserver(
            ...
            (updatedValue) => {
                ...
                if (window.sigApi.properties.instanceofPropertyScheme(this._property)) {
                    // scenario 2
                    ...
                    if (window.sigApi.properties.instanceofPropertyUnit(propertyUnit)) {
                        // scenario 2.1
...
DemoApiPropertiesCreatePropertyKeepPropertyStructure.init();
