...
export class LasalRuntimeSigVideo extends LasalRuntimeSigElement {
    ...
    ****************************************************************************************************/
    connectedCallback(): void {
        ...
        this.sigAddEventListener('pause', (e: Event) => {
            this._dispatchPaused(e);
        }, 'jsEvent', this.$.video);
        this.sigAddEventListener('playing', (e: Event) => {
            this._dispatchPlaying(e);
        }, 'jsEvent', this.$.video);
        this.sigAddEventListener('ended', (e: Event) => {
            this._dispatchEnded(e);
        }, 'jsEvent', this.$.video);

        // prevent default click event on video, so we can implement this functionality with 'tap'
        this.sigAddEventListener('click', (event: Event) => {
    ...
    ****************************************************************************************************/
    play(): void {
        (this.$.video as HTMLMediaElement).play().catch((error) => {
    ...
    ****************************************************************************************************/
    pause(): void {
        (this.$.video as HTMLMediaElement).pause();
    }
    ...
    ****************************************************************************************************/
    stop(): void {
        ...
        (this.$.video as HTMLMediaElement).pause();
        (this.$.video as HTMLMediaElement).currentTime = 0;
    }
