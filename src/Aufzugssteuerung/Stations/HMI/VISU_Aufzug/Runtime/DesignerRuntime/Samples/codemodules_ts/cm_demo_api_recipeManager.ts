...
class DemoApiRecipeManager {
     ...
     */
    _registerEvents() {
        const evtRecipeSave = window.sigApi.events.getUserDefinedInternalEvent('evtRecipeSave');
        if (evtRecipeSave) {
        ...
        const evtRecipeLoad = window.sigApi.events.getUserDefinedInternalEvent('evtRecipeLoad');
        if (evtRecipeLoad) {
            window.sigApi.eventMediator.subscribe(
                evtRecipeLoad,
                () => {
    ...
    _onRecipeLoad() {
        ...
        const filter1 = 0;
        const filter2 = 0;
        window.sigApi.recipeManager.load(dpne, filter1, filter2).then((res) => {
    ...
    _onRecipeSave() {
        ...
        const filter1 = 0;
        const filter2 = 0;
        window.sigApi.recipeManager.save(dpne, headertext, filter1, filter2).then((res) => {
